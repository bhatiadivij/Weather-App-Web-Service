...
async function getLatLong(street, city, state) {
    try {
        url = "https://maps.googleapis.com/maps/api/geocode/json?address="+encodeURIComponent(street)+","+encodeURIComponent(city)+","+encodeURIComponent(state)+"&key="+process.env.GEOCODE_API_KEY
...
async function getStateSeal(state) {
    try {
        url = "https://www.googleapis.com/customsearch/v1?q=" + state + "%20State%20Seal&cx=" + process.env.SEARCH_ENGINE_ID + "&imgSize=huge&imgType=news&num=1&searchType=image&key=" + process.env.CUSTOM_SEARCH_API_KEY
        const json_data = await callAPI(url)
...
async function autocomplete(input) {
    try {
        url = "https://maps.googleapis.com/maps/api/place/autocomplete/json?input=" + input + "&types=(cities)&language=en&key=" + process.env.PLACES_API_KEY
        const json_data = await callAPI(url)
        return json_data.data
    } catch (err) {
        console.log(err)
...
router.get('/auto_complete', async (req, res) => {
...
router.get('/weather_search', async (req, res) => {
...
router.get('/weather_search_with_coordinates', async (req, res) => {
    lat = req.query.lat
    lng = req.query.lng
    state = req.query.state
    city = req.query.city
...
router.get('/weather_detail', async (req, res) => {
    lat = req.query.lat
    lng = req.query.lng
    time = req.query.time
    try {
